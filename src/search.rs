...
#[cfg(feature = "sphere-search")]
fn sphere_perturb(base: i32, hash: u64, ply: usize, depth: i32, mv: Move) -> i32 {
    let mut s = hash
        ^ (ply as u64).wrapping_mul(0x9e3779b97f4a7c15)
        ^ (depth as u64).wrapping_mul(0x6c62272e07bb0142)
        ^ (mv.0 as u64).wrapping_mul(0xd6e8feb86659fd93);
    s ^= s >> 30;
    s = s.wrapping_mul(0xbf58476d1ce4e5b9);
    s ^= s >> 27;
    s = s.wrapping_mul(0x94d049bb133111eb);
    s ^= s >> 31;
    let noise = SPHERE_PI[(s & 0xff) as usize] as i32 - 128;
...
fn alpha_beta(
    ...
    if ply >= 1 && depth <= SPHERE_MAX_DEPTH && board.all_occupancy.count_ones() > 5 {
        ...
        for i in 0..list.len() {
            let m = list.moves[i];
            in_tail[i] = !m.is_capture()
                && !m.is_promotion()
                && !m.is_en_passant()
                && scores[i] < 650_000;
        }
        for _ in 0..SPHERE_QUIET_THRESHOLD {
            let mut top: Option<usize> = None;
            for i in 0..list.len() {
                if in_tail[i] && top.is_none_or(|t| scores[i] > scores[t]) {
                    top = Some(i);
                }
            }
            match top {
                Some(t) => in_tail[t] = false,
                None => break,
            }
        }
        for i in 0..list.len() {
            if in_tail[i] {
                scores[i] = sphere_perturb(scores[i], board.hash, ply, depth, list.moves[i]);
            }
        }
    }
